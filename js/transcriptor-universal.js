"use strict";
function Transcribir(texts,rules) {
	texts = texts.toString();
	rules = rules.toString();
	let char1 = rules.charAt(0);
	let char2 = rules.charAt(1);
	if ( char1 === char2 ) { return ["Error en : char1 = char2 ( " + char2 + char2 + " )",rules]; };
	rules = rules.slice(1);
	rules = rules.slice(1);
	let aux1 = "\\";
	let aux2 = "\\";
	...
	while ( ListB[positionB] ) {
		...
		if ( ListB[positionB].replace(char3,"").length !== 1 ) { return ["Error en : " + ListB[positionB],char1 + char2 + rules]; };
